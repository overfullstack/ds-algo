...
@Serializable
data class ListListIntToLList(@SerialName("testcases") val testcases: List<Testcase>) {
  @Serializable
  data class Testcase(
  ...
  companion object {
    ...
    ): List<Pair<Triple<List<Int>, List<Int>, Int>, List<List<Int>>>> {
      val json = Json { explicitNulls = false }
      val testCases =
        jsonFilePaths.flatMap {
          json.decodeFromString<ListListIntToLList>(readFileToString(it)).testcases
        }
