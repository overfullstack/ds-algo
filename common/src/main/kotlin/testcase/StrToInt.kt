...
@Serializable
data class StrToInt(val testcases: List<Testcase>) {
  @Serializable
  ...
  companion object {
    fun parseJsonFileToTestCases(vararg jsonFilePaths: String): List<Pair<String, Int>> {
      val testCases =
        jsonFilePaths.flatMap { Json.decodeFromString<StrToInt>(readFileToString(it)).testcases }
