...
data class TestCases(val testcases: List<Case>) {
  ...
  data class Case(
    ...
    inline fun <reified T> input(slot: Int): T =
      TestCases.decode(inputs[slot - 1].getValue("$slot"))

    /** Decode the 1-based output [slot] into the caller-supplied type. */
    inline fun <reified T> output(slot: Int): T =
      TestCases.decode(output[slot - 1].getValue("$slot"))
  }
  ...
  companion object {
    ...
    fun load(vararg paths: String): List<Case> = paths.flatMap {
      json.decodeFromString<TestCases>(readFileToString(it)).testcases
    }
  }
}
