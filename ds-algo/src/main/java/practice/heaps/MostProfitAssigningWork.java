...
public class MostProfitAssigningWork {
	public int maxProfitAssignment(int[] difficulty, int[] profit, int[] worker) {
		var minHeapForDifficulty =
				new PriorityQueue<>(Comparator.<Integer>comparingInt(i -> difficulty[i]));
		var maxHeapForProfit =
				new PriorityQueue<>(Comparator.<Integer>comparingInt(i -> profit[i]).reversed());
		...
		for (var capacity : worker) {
			while (!minHeapForDifficulty.isEmpty()
					&& capacity >= difficulty[minHeapForDifficulty.peek()]) {
	...
	static void main() {
		...
		System.out.println(
				mostProfitAssigningWork.maxProfitAssignment(
						new int[] {2, 4, 6, 8, 10},
		...
		System.out.println(
				mostProfitAssigningWork.maxProfitAssignment(
						new int[] {64, 88, 97}, new int[] {53, 86, 89}, new int[] {98, 11, 6})); // 89
	}
}
