...
public class MaximumWidthRamp {
	public int maxWidthRamp(int[] nums) {
		...
		for (var i = 0; i < nums.length; i++) {
			if (stk.isEmpty() || nums[i] < nums[stk.peekLast()]) {
				stk.add(i); // ! Push the index
			}
		}
		var maxRampWidth = 0;
		for (var i = nums.length - 1; i >= 0; i--) {
			while (!stk.isEmpty() && nums[i] >= nums[stk.peekLast()]) { // `=` as per problem
				maxRampWidth = Math.max(maxRampWidth, i - stk.removeLast());
			}
		}
		return maxRampWidth;
	}
