...
public class WordLadder2 {
  ...
  // TLE - Inefficient and it's a broad search from begin to end
  public List<List<String>> findLadders2(String beginWord, String endWord, List<String> wordList) {
          ...
          for (var ch = 'a'; ch <= 'z'; ch++) {
            if (curCh != ch) {
              ...
              if (wordSet.contains(nextWord)) {
                ...
                wordSet.remove(nextWord);
                distanceFromBeginWord.put(nextWord, distance);
                successors.computeIfAbsent(curWord, _ -> new HashSet<>()).add(nextWord);
                if (nextWord.equals(endWord)) {
                  endFound = true;
                }
              } else if (distanceFromBeginWord.getOrDefault(nextWord, 0) == distance) {
                successors.computeIfAbsent(curWord, _ -> new HashSet<>()).add(nextWord);
              }
            }
          }
    ...
    return dfsSuccessors(beginWord, endWord, new ArrayDeque<>(), successors);
  }

  private static List<List<String>> dfsSuccessors(
      String curWord, String endWord, ArrayDeque<String> path, Map<String, Set<String>> graph) {
    path.addLast(curWord);
    if (curWord.equals(endWord)) {
      List<List<String>> result = List.of(new ArrayList<>(path));
      path.removeLast();
      return result;
    }
    var results = new ArrayList<List<String>>();
    for (String successor : graph.getOrDefault(curWord, Collections.emptySet())) {
      results.addAll(dfsSuccessors(successor, endWord, path, graph));
    }
    path.removeLast();
    return results;
  }
  ...
  static void main() {
    ...
    var result1b = wordLadder2.findLadders2("hit", "cog", test1);
    System.out.println("Test 1 - findLadders: " + result1a);
    System.out.println("Test 1 - findLadders2: " + result1b);
    System.out.println("Test 1 - Results match: " + result1a.equals(result1b));
    System.out.println();

    // Test case 2: No path case
    var test2 = List.of("hot", "dot", "dog", "lot", "log");
    var result2a = wordLadder2.findLadders("hit", "cog", test2);
    var result2b = wordLadder2.findLadders2("hit", "cog", test2);
    System.out.println("Test 2 - findLadders: " + result2a);
    System.out.println("Test 2 - findLadders2: " + result2b);
    System.out.println("Test 2 - Results match: " + result2a.equals(result2b));
    System.out.println();
    ...
    var result3b = wordLadder2.findLadders2("red", "tax", test3);
    System.out.println("Test 3 - findLadders: " + result3a);
    System.out.println("Test 3 - findLadders2: " + result3b);
    System.out.println("Test 3 - Results match: " + result3a.equals(result3b));
    System.out.println();
    ...
    var result4b = wordLadder2.findLadders2("aaaaa", "ggggg", test4);
    long time2 = System.currentTimeMillis() - startTime;

    System.out.println("Test 4 - findLadders: " + result4a.size() + " paths, " + time1 + "ms");
    System.out.println("Test 4 - findLadders2: " + result4b.size() + " paths, " + time2 + "ms");
    System.out.println("Test 4 - Results match: " + result4a.equals(result4b));
    System.out.println("Test 4 - Performance: findLadders2 is " + (time1 > 0 ? (double) time2 / time1 : "N/A")
        + "x the speed of findLadders");
  }
}
