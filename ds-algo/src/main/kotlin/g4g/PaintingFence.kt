...
fun numOfWaysToPaint(noOfSlots: Int, numOfColors: Int): Int {
  ...
  repeat(noOfSlots - 1) {
    // Atmost 2 posts can have same color, so painting this with last one with same color is same as
    // painting (n-1) with different colors.
    // = `paintDifferentColor` instead of `totalNoOfWaysToPaintTillNow`
    // If `n` and `n-1` hv same color than `n-1` and `n-2` must have different colors. So not
