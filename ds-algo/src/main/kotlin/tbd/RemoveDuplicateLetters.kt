 ...
 *
 * The key insight is to use a greedy approach with a stack to maintain lexicographical order. We
 * process characters from left to right, and for each character, we decide whether to include it in
 * our result based on:
 ...
 *
 * Why this approach? We need to ensure the result is lexicographically the smallest while
 * maintaining that each character appears exactly once. A stack allows us to "backtrack" and remove
 * characters when we find a better lexicographical option.
 */
fun removeDuplicateLetters(s: String): String {
  ...
  s.forEach { char -> charCount[char - 'a']++ }
  ...
  s.forEach { char ->
    ...
    while (result.isNotEmpty() && char < result.last() && charCount[result.last() - 'a'] > 0) {
