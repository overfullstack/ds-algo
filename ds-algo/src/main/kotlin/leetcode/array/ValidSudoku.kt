...
private fun Array<CharArray>.isValidRows(): Boolean = all { row ->
  row.asSequence().filter { it.isDigit() }.map { it - '0' }.hasNoDuplicates()
}
...
private fun Array<CharArray>.isValidSubBoxes(): Boolean =
  (0..6 step 3).all { startRow ->
    (0..6 step 3).all { startCol -> getSubBoxDigits(startRow, startCol).hasNoDuplicates() }
  }
...
private fun Array<CharArray>.getSubBoxDigits(startRow: Int, startCol: Int): Sequence<Int> =
  (startRow until startRow + 3)
    .asSequence()
    .flatMap { row -> (startCol until startCol + 3).asSequence().map { col -> this[row][col] } }
    .filter { it.isDigit() }
    .map { it - '0' }
...
private fun Sequence<Int>.hasNoDuplicates(): Boolean {
  ...
  return all { digit -> digit in 1..9 && seen.add(digit) }
}
