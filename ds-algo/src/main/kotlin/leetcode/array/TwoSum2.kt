...
tailrec fun twoSum(
  ...
  when {
    ...
    nums[result.first] + nums[result.second] < target ->
      twoSum(nums, target, result.first + 1 to result.second)
    nums[result.first] + nums[result.second] > target ->
      twoSum(nums, target, result.first to result.second - 1)
    else -> intArrayOf(result.first + 1, result.second + 1) // +1 is problem specific
  }
// If problem asks for all unique pairs, after finding one, increment and decrement first and second
// at the same time.
