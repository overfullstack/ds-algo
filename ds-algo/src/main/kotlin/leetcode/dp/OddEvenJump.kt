 ...
 * This problem description is not clear, but it expects you to jump alternatively, 
 * and it expects only indices with odd jumps  
 */
fun oddEvenJumps(arr: IntArray): Int {
  ...
  for (i in arr.lastIndex-1 downTo 0) {
    ...
    valueToIndex.ceilingEntry(currentValue)?.let {
      canJump[i][1] = canJump[it.value][0]
    }
    valueToIndex.floorEntry(currentValue)?.let {
      canJump[i][0] = canJump[it.value][1]
    }
