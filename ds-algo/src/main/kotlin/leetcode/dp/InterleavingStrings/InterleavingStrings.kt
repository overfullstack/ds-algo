...
fun interLeavingStrings(str1: String, str2: String, str3: String): Boolean {
  ...
  for (i in 1..table.lastIndex) { // Is `str3` made of only `str1`
    table[0] =
      table[0] &&
        str3[i - 1] == str1[i - 1] // Analogous to first column in 1D, accumulating to index `0`
  }
  ...
  for (i in 1..str1.length) {
    for (j in 1..str2.length) {
      // Excluding char from str2, If u imagine a 2D array, `table[j]` indicates we are in the same
      // column and previous row.
      // * `table[j]` here represents previous results with `i-1` chars from `str1` and `j` chars
      // from `str2`
      val isCharFromStr1 =
