 ...
 * one-side sorted. It's easy if target is found on that side. Or problem as usual. 4 paths:
 * - Left is Sorted
 *     - target in left range
 *     - Right side has rotation problem, deal with it recursively.
 * - Right is Sorted
 *     - target in right range
 *     - Left side has rotation problem, deal with it recursively.
 */
tailrec fun search(nums: IntArray, target: Int, left: Int = 0, right: Int = nums.lastIndex): Int {
  ...
  return when {
    ...
    nums[left] < nums[mid] ->
      when { // If left sorted, search for target in left range.
        ...
        // side.
        // Problem restarts from right side.
