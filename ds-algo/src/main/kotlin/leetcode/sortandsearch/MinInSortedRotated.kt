...
tailrec fun findMin(nums: IntArray, left: Int = 0, right: Int = nums.lastIndex): Int {
  // * All rotated elements are less than non-rotated elements. nums[left] > nums[right]
  ...
  if (nums[left] <= nums[right]) { // ! Left stepped on the first element of the rotated part
    return nums[left]
  }
  ...
  return when {
    ...
    // ! Means when left and right are next to each other, mid is on left
