...
private fun Int.topologicalSort(
  ...
  diGraph[this]?.asSequence()?.flatMap {
    when {
      // `visited.apply { add(it) }` coz we need to retain it across recursions. `visitedInBranch +
      // it` no need to retain.
      it !in visited ->
        it.topologicalSort(diGraph, visited.apply { add(it) }, visitedInBranch + it) + it
