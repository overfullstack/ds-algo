 ...
 * 
 * The key insight is that this problem reduces to finding the longest increasing subsequence
 * after sorting the envelopes. We sort by width in ascending order, and for envelopes with
 * the same width, we sort by height in descending order.
 * 
 * Why this approach? After sorting, we can use the LIS algorithm on heights to find the
 * maximum number of envelopes that can be nested. The descending height sort for same width
 * ensures we don't incorrectly count envelopes with equal widths as nestable.
 * 
 ...
 * 
 * This is an optimized O(n log n) approach that maintains a "tails" array where
 * tails[i] represents the smallest possible tail value for a subsequence of length i+1
 */
 ...
 * 
 * This function finds the first position where tails[pos] >= num
 * If no such position exists, it returns tails.size
 */
