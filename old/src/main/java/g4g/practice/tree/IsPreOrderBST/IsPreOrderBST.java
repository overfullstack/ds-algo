package g4g.practice.tree.IsPreOrderBST;
...
public class IsPreOrderBST {
  ...
  private static boolean isPreOrderBST(int[] preArr) {
    ...
    for (var ele : preArr) {
      // First left tree will be checked with MIN_VALUE, then once right tree is encountered rest of
      // the nodes are
      ...
      while (!stk.isEmpty()
          && ele > stk.peek()) { // if a greater element is found, stack is dug until the root
        root = stk.pop();
      }
      stk.push(ele);
    }
