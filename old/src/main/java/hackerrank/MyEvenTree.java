...
public class MyEvenTree {
	static void main() {
		...
		for (var i = vertexCount; i > 0; i--) { // y doesn't top-down approach work?
			...
			if (graph.getAllConnectedVertices(i).iterator().hasNext()) {
				for (int adj : graph.getAllConnectedVertices(i)) {
					...
					if (adjChildCount != null) childCount += adjChildCount;
					childCount++;
				}
			}
		...
		// First condition is obvious, that both sides of an edge got even numbers (That is vertices got
		// odd children), sides can be seperated by removing the edge
		// Here we consider one child as parent and other as child.
		for (var i = 0; i < edgeCount; i++) {
			if ((children.get(edges[i][0]) % 2 != 0 && children.get(edges[i][1]) % 2 != 0)
					|| (children.get(edges[i][1]) - children.get(edges[i][0]) % 2 != 0
							&& children.get(edges[i][0]) % 2 != 0)) {
				edgesRemoveCount++;
			}
		}
