...
public class MaximumSubArray {
  ...
  public static Subarray findMaximumSubarrayContiguous(List<Integer> arr) {
    ...
    for (var i = 0; i < arr.size(); i++) {
      sum += arr.get(i);
      // Only a bigger negative, can nullify the sum that was accumulated till now. It essentially
      // is an indication for end of this chain
      // If current negative hasn't made the sum negative, the sum survives until a big positive
      // bumps up its value.
      if (sum < 0) {
        sum = 0;
        startIndex = i + 1;
