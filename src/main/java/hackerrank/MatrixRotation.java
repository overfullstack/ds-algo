...
public class MatrixRotation {
    ...
    private static void rotateLayer(long[][] mat, int layer, int layerRow, int layerCol, int layerPerimeter, int layerRotateCount) {
        for (var currentRotateOriginVal = 1; currentRotateOriginVal <= gcd(layerPerimeter, layerRotateCount); currentRotateOriginVal++) {
            ...
            while (true) {
                ...
                var nextIndexVal = (currentRotateOriginVal + (i * layerRotateCount) % layerPerimeter) % layerPerimeter;
                if (nextIndexVal == 0) {
                    nextIndex = new int[2];
                    nextIndex[0] = 1 + layer;
                    nextIndex[1] = layer;
