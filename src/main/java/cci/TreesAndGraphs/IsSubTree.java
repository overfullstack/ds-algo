...
public class IsSubTree {
    ...
    private static boolean isSubTree(TreeNode pt, TreeNode st) {
        if (pt == null) return false; // Since in this method we are navigating through pt if data doesn't match

        if (pt.data == st.data) {
            // it is kept in if rather than returning directly coz, even if one case fails to be a subtree in the middle,
