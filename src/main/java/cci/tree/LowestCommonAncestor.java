...
public class LowestCommonAncestor {
  ...
  private static TreeNode commonAncestorOptimized(TreeNode root, TreeNode p, TreeNode q) {
    ...
    if (x != null && x != p
        && x != q) { // This fails when in trace back, after p or q is found. This avoids spawning
      // right when common ancestor is found.
      return x; // Which means it originated from * Found Ancestor *
    }
    // Once you find p, you search the right side of the tree for q,
    // if q not found, this below call returns null, and the entire computation returns x as per
    // `return x == null ? y : x;`
