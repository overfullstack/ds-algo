...
public class Maze {
    ...
    private static void printPathIfReachedDoorOut(int m, int n, int[][] maze, int i, int j, int k, int l) {
    ...
    private static boolean isSolved(int[][] maze, int i, int j, int k, int l, int m, int n, boolean[][] visited) {
        if (i == k && j == l) {
            visited[i][j] = true;
            return true;
        }
        ...
        if (isValidMove(maze, i, j, m, n, visited)) {
            visited[i][j] = true;
            // Total 8 directions
            isSolved = isSolved(maze, i + 1, j, k, l, m, n, visited) || isSolved(maze, i, j + 1, k, l, m, n, visited)
                    || isSolved(maze, i - 1, j, k, l, m, n, visited) || isSolved(maze, i, j - 1, k, l, m, n, visited)
                    || isSolved(maze, i + 1, j + 1, k, l, m, n, visited) || isSolved(maze, i - 1, j - 1, k, l, m, n, visited)
                    || isSolved(maze, i + 1, j - 1, k, l, m, n, visited) || isSolved(maze, i - 1, j + 1, k, l, m, n, visited);
    ...
    private static boolean isValidMove(int[][] maze, int i, int j, int m, int n, boolean[][] visited) {
