package g4g.Practice.Trees.ConnectAtSameLevelForInCompleteTree;
...
public class ConnectAtSameLevelForIncompleteTree {
    ...
    private static void connect(Node root) {
        ...
        if (root.left != null) { // If no nextRight found, making connections in next level.
            ...
            connect(root.left); // Sweep from left to right horizontally on same level, for trampoline effect, in the next recursion we shall go to right on the same level
