package g4g.Practice.Trees.ConnectAtSameLevelInCompleteTree;
...
public class ConnectAtSameLevelForInCompleteTree {
    ...
    private static void connect(Node root) {
        ...
        // We cover right side of the tree first, making sure all those have nextRight pointers.
        ...
        if (root.left != null) { // If no nextRight found, making connections in next level.
            if (root.right != null) {
                ...
                root.right.nextRight = getNext(root); // For this to happen, all the right side of our tree must be linked.
            } else {
                root.left.nextRight = getNext(root);
            }
            connect(root.left); // Although we recur left, the nextRight check at the top steers us towards right side of the tree.
