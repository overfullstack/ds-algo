package g4g.Regular.SortAndSearch.UniqueInSortedArray;
...
public class UniqueInSortedArray {
  ...
  private static void searchUnique(int[] arr, int low, int high) {
    ...
    // this can also be replaced by low==high, but the below condition can detect early, in cases
    // like unique
    ...
    if (mid % 2 == 0) { // If mid even, means we landed on an odd position
      if (arr[mid]
          == arr[mid + 1]) { // If this condition satisfies in odd position, left part is clear, so
        // search right side
