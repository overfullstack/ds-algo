...
public class SubSetSum {
    ...
    private static boolean isSumPresent2(int[] set, int sum) {
        ...
        for (var i = 1; i <= sum; i++) {
            for (var j = 1; j <= len; j++) {
                table[i][j] = table[i][j - 1]; // Exclude case - if we exclude this, check if we make this sum with
                // other elements
                if (set[j - 1] <= i) { // If current number we are checking is less than or equal to sum we need
                    table[i][j] |= table[i - set[j - 1]][j]; // Include case table[i-set[j-1]][j] - if we include this,
