package g4g.practice.tree.BottomViewTree;
...
public class BottomViewTree {
  ...
  private static void constructBottomView(
      TreeNode root, Map<Integer, TreeNode> map, int horizontalDistance, int height) {
    if (root == null) {
      return;
    }
    // Top-Down
    ...
    if (node == null
        || node.level <= height) { // Replace if node not present, or node is of lesser height.
