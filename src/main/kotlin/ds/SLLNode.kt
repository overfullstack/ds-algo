...
data class SLLNode(var value: Int, var next: SLLNode? = null) {
    ...
    companion object {
        ...
        fun of2(values: IntArray, index: Int = 0): SLLNode =
            if (index == values.lastIndex) SLLNode(values[index]) else SLLNode(values[index], of2(values, index + 1))
    }
}

fun SLLNode.toArray(): IntArray = intArrayOf(value, *(next?.toArray() ?: intArrayOf()))

tailrec fun SLLNode.getNodeForValue(valToFind: Int): SLLNode? = if (value == valToFind) this else next?.getNodeForValue(valToFind)
