...
fun getSkyline(buildings: List<Triple<Int, Int, Int>>): Array<Pair<Int, Int>> {
...
fun skylineView(buildings: List<Triple<Int, Int, Int>>, left: Int, right: Int): Array<Pair<Int, Int>> {
    if (left == right) {
        return arrayOf(
            Pair(buildings[left].first, buildings[left].second),
...
private fun merge(leftSkyline: Array<Pair<Int, Int>>, rightSkyline: Array<Pair<Int, Int>>): Array<Pair<Int, Int>> {
    var i = 0
    var j = 0
    var h1 = 0
    var h2 = 0
...
private fun MutableList<Pair<Int, Int>>.append(pair: Pair<Int, Int>) {
    if (isEmpty()) {
        add(pair)
        return
    }
...
fun getSkyline(buildings: Array<IntArray>): List<List<Int>> {
    return getSkyline(buildings.map { Triple(it[0], it[1], it[2]) }).map { it.toList() }.toList()
}

fun main() {
    val buildings = readLine()!!.split(",").map { it.trim().toInt() }.chunked(3).map { Triple(it[0], it[1], it[2]) }
    getSkyline(buildings).forEach { print("$it ") }
}