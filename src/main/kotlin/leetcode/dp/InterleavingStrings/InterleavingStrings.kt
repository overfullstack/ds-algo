...
fun interLeavingStrings(str1: String, str2: String, str3: String): Boolean {
    ...
    for (i in 1..str1.length) {
        for (j in 1..str2.length) {
            val includeCharFromStr1 = table[j] && str3[i + j - 1] == str1[i - 1] // Excluding char from str2, If u image a 2D array, `table[j]` indicates we are in the same column and previous row.
            val includeCharFromStr2 = table[j - 1] && str3[i + j - 1] == str2[j - 1] // Including char from str2
