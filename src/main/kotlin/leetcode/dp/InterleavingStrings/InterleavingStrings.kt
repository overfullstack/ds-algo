...
fun interLeavingStrings(str1: String, str2: String, str3: String): Boolean {
    ...
    for (i in 1..table.lastIndex) { // Is `str3` made of only `str1`
        table[0] = table[0] && str3[i - 1] == str1[i - 1] // Analogous to first column in 1D, accumulating to index `0`
    }
    ...
    for (i in 1..str1.length) {
        for (j in 1..str2.length) {
            ...
            val isCharFromStr1 = table[j] && (str3[i + j - 1] == str1[i - 1]) // -1 as 0 is taken for empty str1, str2
