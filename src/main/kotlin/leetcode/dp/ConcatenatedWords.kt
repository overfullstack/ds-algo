...
fun findAllConcatenatedWordsInADict(words: Array<String>): List<String> {
  ...
  val wordLengthStats =
    nonEmptyWords.fold(IntSummaryStatistics()) { stats, word ->
      stats.also { it.accept(word.length) }
    }
  return nonEmptyWords
    .asSequence()
    .filter {
      it.length in wordLengthStats.min * 2..wordLengthStats.max
    } // If it has to be formed using from words within
    .filter {
...
private fun isWordPresent(word: String, wordDictSet: Set<String>, minLength: Int) =
  word.indices
    .drop(minLength - 1)
    .fold(mutableListOf(-1)) { wordEndIndices, wordIndex ->
      wordEndIndices.also {
