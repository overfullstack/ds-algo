...
fun findWords(board: Array<CharArray>, words: Array<String>): List<String> {
  ...
  return board.indices.flatMap { row ->
    board[0].indices.flatMap { col ->
      (trie.children[board[row][col] - 'a']?.findWords(board, row to col) ?: emptySet())
...
private fun TrieNode.findWords(
  ...
  visited: Array<BooleanArray> = Array(board.size) { BooleanArray(board[0].size) }.apply {
    this[gridPoint.first][gridPoint.second] = true
  } // * This visited is specific to this DFT originated at this `gridPoint`. No global Visited, as search from other `gridPoints` can overlap.
): Set<String> =
