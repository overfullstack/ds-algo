...
package leetcode.arrays.MinWindowSubString
...
fun minWindow(str: String, pattern: String): String {
    ...
    for ((strIndex, letter) in str.withIndex()) {
        ...
        // Once matchCount meets patternLength, it always goes inside this if condition. Nothing touches matchCount again.
        if (matchCount == pattern.length) {
            ...
            while (!patternLetterFreq.containsKey(str[startIndex]) || strLetterFreq[str[startIndex]]?.let { it > patternLetterFreq[str[startIndex]] ?: 0 } == true) {
