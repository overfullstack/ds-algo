...
fun printCombinations(str: String, combination: String, startIndex: Int) {
  for (index in
    startIndex until
      str.length) { // No need of used[], as startIndex prevents usage all those that are already
...
fun combinations(str: String, combination: String = "", startIndex: Int = 0): List<String> =
  (startIndex..str.lastIndex)
    .flatMap { // It's like each index is supposed to produce a list per branch.
