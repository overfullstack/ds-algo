 ...
 * 
 * The key insight is that for an array of size n, the first missing positive integer
 * must be in the range [1, n+1]. If all numbers from 1 to n are present, then n+1 is missing.
 * 
 * We use the array indices as a hash set - if we see a positive number k in range [1,n],
 * we mark position k-1 as "visited" by making the value negative.
 * 
 * Why this approach? Because we need O(1) space, so we can't use a separate hash set.
 * The array itself becomes our tracking mechanism.
 */
